setwd("C:/Users/Rene/Dropbox/Pinsky_Lab/Transcriptome_Proj/R_scripts/")
getwd()
...
#Create POD
omega_realdata <- as.matrix(read.table("core1_mat_omega.cov")) #Read in posterior distribution of covariance matrix
pi.beta.coef <- read.table("core1_summary_beta_params.out", h=T)$Mean #Read in posterior distributions of a(pi) and b(pi)
geno <- geno2YN("clownfish_data_all.geno") #Read in allele counts in BayPass format to convert to total allele counts (per SNP)
simulate.baypass(omega.mat=omega_realdata, nsnp=5729,sample.size=geno$NN,beta.pi=pi.beta.coef,pi.maf=0,suffix="aclarkiipods") #simulate 5729 loci POD

Xtx_realdata <- read.table("core1_summary_pi_xtx.out", h=T)
plot(Xtx_realdata$M_XtX)

#read in omega estimate from real data
#omega_realdata <- as.matrix(read.table("core1_mat_omega.cov"))
pop.names = c("J", "P", "N")
dimnames(omega_realdata) <- list(pop.names, pop.names)

#compute and visualize correlation matrix
cor.mat_realdata <- cov2cor(omega_realdata)
corrplot(cor.mat_realdata, method="color", mar=c(2,1,2,2)+0.1, main=expression("Correlation map based on" ~hat(Omega)))

#compare estimate of omega from POD & real data
omega_POD <- as.matrix(read.table("POD1_mat_omega.out"))
plot(omega_POD, omega_realdata)
abline(a=0, b=1)

#compare estimate of a_pi & b_pi hyperparameters from POD & real data
pi.beta.coef_realdata <- read.table("core1_summary_beta_params.out", h=T)$Mean
pi.beta.coef_POD <- read.table("POD1_summary_beta_params.out", h=T)$Mean
plot(pi.beta.coef_POD, pi.beta.coef_realdata)
abline(a=0, b=1)

#XtX calibration
Xtx_POD <- read.table("POD1_summary_pi_xtx.out", h=T)$M_XtX
POD_0.01_thresh <- quantile(Xtx_POD, probs=0.99)
plot(Xtx_realdata$M_XtX)
abline(h=POD_0.01_thresh, lty=2)