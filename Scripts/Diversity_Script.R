...
SNPs_mac1 <- read.genetix("../../VCFs_and_PLINK/mac1_genetix.gtx")
allele_counts_mac1 <- read_table2("Data/clownfish_mac1_geno.txt", col_names = TRUE)
...
######## Compute diversity statistics w/only SNPs w/mac >/= 1 ########

#format data
snps_mac1 <- c(1:5718) #create column for indexing
locnames_mac1$snp_id <- snps_mac1
allele_counts_mac1$snp_id <- snps_mac1

allele_counts_mac1 <- merge(allele_counts_mac1, locnames_mac1, by = "snp_id") #merge allele_counts and locnames so know which loci corresponds with what allele frequencies
allele_counts_mac1 <- allele_counts_mac1[,2:8] #remove snp_id index

#calculate Ho per locus in each pop
sum_stats_mac1 <- basic.stats(SNPs_mac1) #hierfstat function --> gives Ho, Hs, Fis on locus basis/pop as well as some pop-wide summary stats (Hs from Nei & Chesser 1983)
J_Ho_mac1 <- data.frame(sum_stats_mac1$Ho[,1]) #pull out Ho for J pop
I_Ho_mac1 <- data.frame(sum_stats_mac1$Ho[,2])
P_Ho_mac1 <- data.frame(sum_stats_mac1$Ho[,3])

#calculate Hs (~He) per locus in each pop
J_Hs_mac1 <- apply(allele_counts_mac1[,1:2], 1, heterozygosity) #pegas heterozygosity function to calculate Nei's average gene diversity w/correction for small sample sizes
P_Hs_mac1 <- apply(allele_counts_mac1[,3:4], 1, heterozygosity)
I_Hs_mac1 <- apply(allele_counts_mac1[,5:6], 1, heterozygosity)

#calculate Fis per locus in each pop
J_header <- c("J_Ho", "J_Hs")
J_diversity_mac1 <- data.frame(J_Ho_mac1, J_Hs_mac1) #combine het metrics into same data frame
colnames(J_diversity_mac1) <- J_header #set header
J_diversity_mac1$J_Fis <- 1 - (J_diversity_mac1$J_Ho/J_diversity_mac1$J_Hs) #calculate Fis per locus

P_header <- c("P_Ho", "P_Hs")
P_diversity_mac1 <- data.frame(P_Ho_mac1, P_Hs_mac1)
colnames(P_diversity_mac1) <- P_header
P_diversity_mac1$P_Fis <- 1 - (P_diversity_mac1$P_Ho/P_diversity_mac1$P_Hs)

I_header <- c("I_Ho", "I_Hs")
I_diversity_mac1 <- data.frame(I_Ho_mac1, I_Hs_mac1)
colnames(I_diversity_mac1) <- I_header
I_diversity_mac1$I_Fis <- 1 - (I_diversity_mac1$I_Ho/I_diversity_mac1$I_Hs)

#combine to one data frame and write out
diversity_tot_mac1 <- data.frame(J_diversity_mac1, P_diversity_mac1, I_diversity_mac1)
diversity_tot_mac1$snp_id <- snps_mac1
diversity_tot_mac1 <- merge(diversity_tot_mac1, locnames_mac1, by="snp_id")
diversity_tot_mac1 <- diversity_tot_mac1[2:11]
write.csv(diversity_tot_mac1, file = "Data/mac1_diversity.csv")
...
J_Fis <- data.frame(diversity_tot_mac1$J_Fis)
  colnames(J_Fis) <- c("Fis")
J_Fis <- subset(J_Fis, !is.na(J_Fis$Fis)) #remove NAs
J_Fis_addback <- data.frame(matrix(0, nrow = 2221, ncol = 1)) #add back NAs as 0s
  colnames(J_Fis_addback) <- c("Fis")
J_Fis <- rbind(J_Fis, J_Fis_addback) #rbind dataframes to get full list of Fis values
J_Fismean <- mean(J_Fis$Fis) #calculate mean (-0.033956)
...
P_Fis <- data.frame(diversity_tot_mac1$P_Fis)
  colnames(P_Fis) <- c("Fis")
P_Fis <- subset(P_Fis, !is.na(P_Fis$Fis)) #remove NAs
P_Fis_addback <- data.frame(matrix(0, nrow = 1452, ncol = 1)) #add back NAs as 0s
  colnames(P_Fis_addback) <- c("Fis")
P_Fis <- rbind(P_Fis, P_Fis_addback) #rbind dataframes to get full list of Fis values
...
N_Fis <- data.frame(diversity_tot_mac1$I_Fis)
  colnames(N_Fis) <- c("Fis")
N_Fis <- subset(N_Fis, !is.na(N_Fis$Fis)) #remove NAs
N_Fis_addback <- data.frame(matrix(0, nrow = 2267, ncol = 1)) #add back NAs as 0s
  colnames(N_Fis_addback) <- c("Fis")
N_Fis <- rbind(N_Fis, N_Fis_addback) #rbind dataframes to get full list of Fis values
...
#write out data
write.csv(Fis_sum, "Data/Fis_cis_mac1.csv")

###############################################################################################################################################

######## (Optional) inbreeding measure (Fis) with adegenet (SNPs w/mac>2) ########

#set population names
popNames(SNPs_mac1) <- c("Japan", "Indonesia", "Philippines")
popNames(SNPs_mac1) #check to make sure popnames are right

#subset dataframe to each population
J_inds <- SNPs_mac1[1:8, ]
J_sum <- summary(J_inds) #get summary to make sure subsetting correctly
I_inds <- SNPs_mac1[9:15, ]
I_inds <- SNPs_mac1[16:25, ]

#inbreeding measure
J_inbred <- inbreeding(J_inds) #creates likelihood distribution for Fis of each locus
J_inbred_mean_mac1 <- sapply(J_inbred, mean) #mean likelihood
hist(J_inbred_mean_mac1) #plot
